...
namespace AppLogin.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //    var userid1 = UserManager.FindByEmail(model.Email).Id;
        //    if (!UserManager.IsEmailConfirmed(userid1))
        //    {
        //        var authenticationManager = HttpContext.GetOwinContext().Authentication;
        //        authenticationManager.SignOut();

        //        ModelState.AddModelError("", "Debes verificar el correo antes de ingresar");
        ...
        //            var userid = UserManager.FindByName(model.Password).Id;
        //            if (!UserManager.IsEmailConfirmed(userid))
        //            {
        //                var authenticationManager1 = HttpContext.GetOwinContext().Authentication;
        //                authenticationManager1.SignOut();
...

        {
            ...
            var userid = UserManager.FindByEmail(model.Email).Id;  // it gives null exception.
            if (!UserManager.IsEmailConfirmed(userid))
            {
                ModelState.AddModelError("", "Debes verificar el correo antes de ingresar");
                //  return View(model);
            ...
            switch (result)
            {
                ...
                case SignInStatus.Failure:
                default:
                    ModelState.AddModelError("", "Intento de inicio de sesion invalido");
                    return View(model);
            }
        }
    ...
    [AllowAnonymous]
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                   // Comentar esta linea para que espere confirmacion antes de loguear// 
